...
//        processRequest(request, response)
        ...
        Emp_ChamCongDAO dao = new Emp_ChamCongDAO();
              try {
                  ...
                  Emp_ChamCong cc = new Emp_ChamCong(maNV, ngay, gioVao, gioRa, sogiolam);
                  dao.them(cc);

                  request.getSession().setAttribute("dataCC", cc);
                  HttpSession session = request.getSession();
                  session.setAttribute("chamcong", true);
                  response.sendRedirect("Emp_ChamCong.jsp");
              } catch (SQLException ex) {
                  Logger.getLogger(Emp_ChamCongServlet.class.getName()).log(Level.SEVERE, null, ex);
              }
    }
